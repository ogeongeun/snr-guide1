// src/pages/SiegeTeamEditPage.jsx
...
export default function SiegeTeamEditPage() {
  const navigate = useNavigate();
  const { id } = useParams(); // /siege/edit/:id

  const postId = useMemo(() => {
    // UUID든 숫자든 그대로 string으로 사용
    return (id || "").trim();
  }, [id]);
  ...
  const [selectedDay, setSelectedDay] = useState(""); // 수정 페이지에서는 DB에서 day를 가져옴
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        ...
        setSelectedDay(dayOrder.includes(day) ? day : day);
        ...
        // slots 채우기
        const nextSlots = Array.from({ length: 5 }, (_, i) => ({
          hero_key: "",
          name: "",
          image: "",
          build: defaultBuild(),
        ...
        for (const m of members || []) {
          ...
          // 이미지 우선순위:
          // 1) members.hero_image
          // 2) heroesList 매칭 image
        ...
        // skill_orders: 배열일 수도 있고, 없을 수도 있음
        const so = Array.isArray(post.skill_orders) ? post.skill_orders : [];
        const one = so[0] || defaultSkillOrder();
  ...
  const validate = () => {
    ...
    if (!selectedDay) return "day 정보를 찾을 수 없습니다.";
    for (let i = 0; i < 5; i++) {
  ...
  const backToDay = () => {
    if (selectedDay) {
      navigate(`/siege/${encodeURIComponent(selectedDay)}`);
      return;
    }
    navigate("/siege");
  };
