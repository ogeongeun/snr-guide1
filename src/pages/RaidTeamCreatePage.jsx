...
import raidBossOptions, { getRaidBossLabel } from "../data/raidBossOptions";
...
export default function RaidTeamCreatePage() {
  const navigate = useNavigate();
  const q = useQuery();

  const boss = q.get("boss") ? decodeURIComponent(q.get("boss")) : "";
  const selectedBossKey = raidBossOptions.some((x) => x.key === boss) ? boss : "";
  ...
  const validate = () => {
    if (!selectedBossKey) return "boss 파라미터가 올바르지 않습니다.";
  ...
  const save = async () => {
    ...
    try {
      ...
      if (memErr) {
        await supabase.from("raid_team_posts").delete().eq("id", postId);
        throw memErr;
      }
      ...
      <div className="rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
          <div className="text-[12px] font-extrabold text-slate-500">
            {getRaidBossLabel(selectedBossKey)}{" "}
            {loadingMe ? <span className="ml-2 text-slate-400">(유저 확인중)</span> : null}
          </div>

          <button
            onClick={save}
            disabled={saving || !selectedBossKey}
